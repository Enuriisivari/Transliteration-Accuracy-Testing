...
def _resolve_browser_executable(p: str | None) -> str | None:
    if p:
    ...
    for command in ("chrome", "chromium", "chromium-browser", "msedge"):
...
def _find_frontend_locators(page):
    input_candidates = [
        page.locator('textarea[placeholder="Input Your Singlish Text Here."]').first,
        page.locator('textarea[placeholder*="Singlish"]').first,
        page.locator("textarea").first,
    ]
    output_candidates = [
        page.locator('div.w-full.h-full.p-3.pb-8.rounded-lg.ring-1.ring-slate-300.whitespace-pre-wrap.overflow-y-auto.bg-slate-50').first,
        page.locator('div.min-h-[140px].p-3.rounded-lg.ring-1.ring-slate-300.whitespace-pre-wrap').first,
    ...
    output_locator = None
    for locator in output_candidates:
        try:
            if locator.count() > 0:
    ...
    if input_locator is None:
        raise RuntimeError("Could not find the Singlish input textarea.")
    if output_locator is None:
        raise RuntimeError("Could not find the output container.")

    action_locator = page.get_by_role("button", name=re.compile(r"^Translate$", re.IGNORECASE)).first
...
def run_test():
    ...
    browser_executable = _resolve_browser_executable(args.browser_executable)
    ...
    with sync_playwright() as p:
        ...
        if not browser_executable:
            print(
                "Error: No local Chromium/Chrome executable found. "
                "Install Chrome/Chromium or pass --browser-executable <path>."
            )
            return
        ...
        input_locator, output_locator, action_locator = _find_frontend_locators(page)
        is_chat = False
